...
# Главное меню с кнопками и эмодзи (красивее!)
...
# Вопросы для теста по журналистике (правильные: A, B, A, C, B)
...
def start(message):
    ...
    if is_user_in_db(user_id):
        # Если уже в базе — сразу меню
        show_menu_and_greeting(message)
        return

    # Отправляем заставку (замените URL на реальный)
    try:
        bot.send_photo(user_id, 'https://example.com/splash.jpg', caption='Добро пожаловать в Медиацентр Марфино! 🌟')
    except:
        pass  # Если URL неверный — просто продолжим без фото
...
def handle_message(message):
    ...
    # Обработка кнопок меню (теперь с эмодзи в тексте)
    if '🎬 Чек-лист для написания закадрового текста с ИИ' == text:
        bot.send_message(user_id, checklist_text_ai)
    elif '📹 Чек-лист для съемки репортажа' == text:
        bot.send_message(user_id, checklist_shooting)
    elif '🔗 Полезные ресурсы' == text:
        bot.send_message(user_id, resources_text)
    elif '📚 Курсы aXIS' == text:
